...
public class ConfigurationArtifactHandler extends AbstractXmlArtifactHandler {
	...
	public void apply(EnvironmentBuildContext context) throws ArtifactHandlerException {
		...
		if (!configuration.exists() || !definition.exists()) {
			context.getLog().debug("Skipping configuration handler, required files not found in " + context.getProjectDirectory());
			return;
		}
			...
			if (field.isList()) {
				...
				if (list != null) {
					for (int i = 0; i < list.size(); i++) {
						replaceNodeValue(context, node(xpath, document, "/configuration/" + entry.getKey() + "[" + (i + 1) + "]/text()"), list.get(i), false);
					}
				}
			}
			else {
				replaceNodeValue(context, node(xpath, document, "/configuration/" + entry.getKey() + "/text()"), entry.getValue(), false);
			}
		}
